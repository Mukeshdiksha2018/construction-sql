...
**Vendor master:** `dbo.Business` where `Type = 1` is the vendor table for the construction app (`Type = 0` is customer). Active/inactive is `Status` (0/1); soft delete is `Status = 3`. CRUD and dropdowns (`VendorSelect`) use `/api/nimble-vendors` (see [`.cursor/plans/nimble-vendors-crud.plan.md`](../../.cursor/plans/nimble-vendors-crud.plan.md)). Construction `vendor_uuid` fields store `Business.ID` as lowercase hex (36 chars, `0000` suffix).
