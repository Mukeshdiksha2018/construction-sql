...
export async function listStockReceiptNotes(
  corporationUuid: string,
  options: { projectUuid?: string; vendorUuid?: string; entryDateFrom?: string; entryDateTo?: string } = {}
) {
  const where: any = { corporation_uuid: corporationUuid, is_active: true }
  ...
  if (options.entryDateFrom || options.entryDateTo) {
    where.entry_date = {}
    if (options.entryDateFrom) where.entry_date.gte = new Date(options.entryDateFrom)
    if (options.entryDateTo) where.entry_date.lte = new Date(`${options.entryDateTo}T23:59:59.000Z`)
  }
...
export async function createStockReceiptNote(input: any) {
  const row = await prisma.stockReceiptNote.create({
    data: {
      corporation_uuid: input.corporation_uuid,
      project_uuid: input.project_uuid ?? null,
      ...
      vendor_uuid: input.vendor_uuid ?? null,
      entry_date: parseDate(input.entry_date),
      received_date: parseDate(input.received_date),
      shipment_date: parseDate(input.shipment_date),
      grn_number: input.grn_number || await generateNextGrnNumber(input.corporation_uuid),
      reference_number: input.reference_number ?? null,
      received_by: input.received_by ?? null,
      nimble_received_by_user_id: input.nimble_received_by_user_id ?? null,
      location_uuid: input.location_uuid ?? null,
      notes: input.notes ?? null,
      status: input.status ?? 'Shipment',
      total_received_amount: toNum(input.total_received_amount),
      attachments: stringifyJson(input.attachments ?? []),
...
export async function updateStockReceiptNote(uuid: string, input: any) {
  const existing = await prisma.stockReceiptNote.findFirst({ where: { uuid, is_active: true } })
  if (!existing) return null

  const updateData: any = {}
  const fields = ['project_uuid', 'purchase_order_uuid', 'receipt_type', 'vendor_uuid', 'grn_number',
    'reference_number', 'received_by', 'nimble_received_by_user_id', 'location_uuid', 'notes', 'status']
  for (const f of fields) {
    if (f in input) updateData[f] = input[f] ?? null
  }
  ...
  if ('total_received_amount' in input) updateData.total_received_amount = toNum(input.total_received_amount)
  ...
  const row = await prisma.stockReceiptNote.update({ where: { uuid }, data: updateData })
  return mapReceiptNoteRow(row)
}
...
export async function replaceReceiptNoteItems(receiptNoteUuid: string, corporationUuid: string | null, projectUuid: string | null, items: any[]) {
