import { replaceReceiptNoteItems } from '../../utils/purchaseOrders'

export default defineEventHandler(async (event) => {
  const body = await readBody(event)
  const { receipt_note_uuid, corporation_uuid, project_uuid, items } = body ?? {}
  if (!receipt_note_uuid) throw createError({ statusCode: 400, statusMessage: 'receipt_note_uuid is required' })
