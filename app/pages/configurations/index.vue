<template>
  <div class="space-y-6">
    <ClientOnly v-if="nimbleIntegrations && visibleTabs.length === 1">
      <section>
        <ConfigurationsTabContent :tab="activeTab" />
      </section>
      <template #fallback>
    ...
    <ClientOnly v-else>
      ...
      >
        <template #content="{ item }">
          <section>
            <ConfigurationsTabContent :tab="String(item.value)" />
          </section>
        </template>
      </UTabs>
      <template #fallback>
...
import ConfigurationsTabContent from '~/components/configurations/ConfigurationsTabContent.vue'
...
function handleTabChange(tab: string | number) {
  ...
  if (nimbleIntegrations.value && NIMBLE_MENU_IDS_BY_CONFIGURATIONS_TAB[validTab.name]) {
    router.push({
      query: {
        ...route.query,
        tab: validTab.name,
        menuId: NIMBLE_MENU_IDS_BY_CONFIGURATIONS_TAB[validTab.name],
      },
    })
  }
  else {
...
function syncTabFromMenuId() {
  ...
  const tabForMenuId = (
    Object.entries(NIMBLE_MENU_IDS_BY_CONFIGURATIONS_TAB) as [string, string][]
...
function ensureCurrentTabVisible() {
  ...
  const menuId = NIMBLE_MENU_IDS_BY_CONFIGURATIONS_TAB[targetTab]
  router.replace({
    query: {
      ...route.query,
      tab: targetTab,
...
watch(() => route.query.tab, (newTab) => {
  if (newTab && typeof newTab === 'string') {
    const validTab = CONFIGURATIONS_TABS.find(t => t.name === newTab)
    if (!validTab) navigateToTab('project-types')
  }
})
