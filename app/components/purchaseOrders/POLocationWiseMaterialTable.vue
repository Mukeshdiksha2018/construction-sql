    ...
    <div v-else-if="hasItems">
        ...
        <table class="min-w-full table-fixed divide-y divide-default/60">
          <thead class="bg-muted/20 text-[11px] font-semibold uppercase tracking-wide text-muted">
            <tr>
              ...
              <th v-if="showBudgetedAmount" class="px-4 py-2 text-right">Material Budgeted Amount</th>
          ...
          <tbody class="divide-y divide-default/60 text-sm text-default">
            ...
            >
              ...
              <td v-if="showBudgetedAmount" class="px-2 py-2 text-right align-middle">
                <div class="w-full max-w-[180px] ml-auto">
                  <div class="grid grid-cols-[auto_auto] items-center justify-end gap-0 rounded-md border border-transparent bg-gray-100 dark:bg-gray-800/40 px-3 py-1.5 po-estimate-value">
                    ...
                    <span class="font-mono text-sm leading-none tracking-tight">
                      {{ formatCurrencyInput(item.material_budgeted_amount || 0) }}
                    </span>
                  </div>
                </div>
              </td>
              <td class="px-2 py-2 text-right align-middle">
                <div class="w-full max-w-[180px] ml-auto">
                  ...
                  />
                  <div
                    v-if="showBudgetedAmount && isAmountExceeded(item, index) && !props.readonly"
      ...
      <div class="md:hidden divide-y divide-default/60">
        ...
        >
          ...
          <div v-if="showBudgetedAmount" class="space-y-1">
            <span class="block text-[11px] uppercase tracking-wide text-muted/80">Material Budgeted Amount</span>
            <div class="grid grid-cols-[auto_auto] items-center justify-end gap-0 rounded-md border border-transparent bg-gray-100 dark:bg-gray-800/40 px-3 py-1.5 po-estimate-value">
...
interface LocationWiseMaterialItem {
  id?: string | number
...
const props = withDefaults(defineProps<{
  title?: string
  description?: string
  items: LocationWiseMaterialItem[]
  loading?: boolean
  error?: string | null
...
const showBudgetedAmount = computed(() => props.showBudgetedAmount !== false)
const showLocationColumn = computed(() => props.showLocationColumn !== false)
...
const poDrafts = reactive<Record<
  number,
  { key: string; poAmountInput: string; amountTouched: boolean }
>>({})
const activeRowIndex = ref<number | null>(null)

const buildDraftKey = (item: LocationWiseMaterialItem, index: number) => {
  return [item.id ?? '', item.cost_code_uuid ?? '', item.location_uuid ?? '', index]
  ...
  (newItems = []) => {
    newItems.forEach((item, index) => {
      ...
      if (!draft || draft.key !== draftKey) {
        poDrafts[index] = { key: draftKey, poAmountInput, amountTouched: false }
        return
      }
      draft.key = draftKey
    ...
    Object.keys(poDrafts).forEach((key) => {
      ...
      if (!Number.isNaN(idx) && !newItems[idx]) delete poDrafts[idx]
    })
  },
  { immediate: true },
)
...
const handleEditSelection = () => emit('edit-selection')
const handleAddRow = (index: number) => emit('add-row', index)
const handleRemoveRow = (index: number) => emit('remove-row', index)
...
const emitPoAmountChange = (index: number, value: string | number | null | undefined) => {
  ...
  const draft =
    poDrafts[index] ||
    (poDrafts[index] = {
      key: buildDraftKey(item as LocationWiseMaterialItem, index),
...
const setActiveRow = (index: number | null) => { activeRowIndex.value = index }
const clearActiveRow = (index: number) => {
  if (activeRowIndex.value === index) activeRowIndex.value = null
}

const isAmountExceeded = (item: LocationWiseMaterialItem, index: number): boolean => {
  if (!showBudgetedAmount.value) return false
  if (!item.material_budgeted_amount || item.material_budgeted_amount <= 0) return false
  ...
  if (draftValue !== undefined && draftValue !== null && draftValue !== '') {
    currentPoAmount = parseNumericInput(draftValue)
  } else if (currentValue !== null && currentValue !== undefined) {
    currentPoAmount = parseNumericInput(currentValue)
  }
  return currentPoAmount > parseNumericInput(item.material_budgeted_amount)
}

const getExceededAmount = (item: LocationWiseMaterialItem, index: number): number => {
  if (!showBudgetedAmount.value) return 0
  if (!item.material_budgeted_amount || item.material_budgeted_amount <= 0) return 0
  const draftValue = poDrafts[index]?.poAmountInput
  const currentValue = item.po_amount
  let currentPoAmount = 0
  if (draftValue !== undefined && draftValue !== null && draftValue !== '') {
  ...
  return Math.max(0, currentPoAmount - parseNumericInput(item.material_budgeted_amount))
}
</script>
