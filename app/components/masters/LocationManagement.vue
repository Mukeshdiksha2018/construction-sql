<template>
  <div>
      ...
      <div class="relative overflow-auto rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
        ...
        <div class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
          <div v-for="i in 5" :key="i">
            <div class="grid grid-cols-5 gap-4 px-2 py-1 text-xs">
    ...
    <div v-else-if="filteredLocations.length">
      <UTable
        ref="table"
    ...
    <UModal
      ...
      description="Configure location details."
      :ui="modalUi"
    ...
    >
      <template #body>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm font-medium text-default mb-2">Location Code</label>
            <UInput
...
const form = ref({ location_name: '', location_code: '', description: '', active: true })
...
const shouldShowPagination = computed(() => filteredLocations.value.length > 10)
const isFormValid = computed(() => form.value.location_name.trim() !== '')
...
function resetForm() {
  form.value = { location_name: '', location_code: '', description: '', active: true }
}
...
async function save() {
  ...
  try {
    const payload = {
      ...
      location_code: form.value.location_code || null,
      description: form.value.description || null,
      active: form.value.active,
    }
    if (editing.value) {
      await store.updateLocation(editing.value.uuid, payload)
      toast.add({ title: 'Success', description: 'Location updated successfully', color: 'success' })
    }
    else {
      await store.createLocation(payload)
      toast.add({ title: 'Success', description: 'Location added successfully', color: 'success' })
    }
    closeModal()
  }
  catch (e: unknown) {
...
async function remove(item: Location) {
  try {
    await store.deleteLocation(item.uuid)
    toast.add({ title: 'Success', description: 'Location deleted successfully', color: 'success' })
  }
  catch (e: unknown) {
    toast.add({ title: 'Error', description: getErrorMessage(e), color: 'error' })
  }
