<template>
  <div>
    ...
    <p v-else class="text-muted text-center py-12">
      No approval checks found.
    </p>

    <UModal
      v-model:open="showModal"
      :title="editing ? 'Edit Approval Check' : 'Add New Approval Check'"
        ...
        <div class="space-y-4">
          ...
          <div>
            ...
            <p class="text-xs text-muted mt-1">
              Toggle to enable or disable this approval check
            </p>
          </div>
        </div>
      </template>
      <template #footer>
        <div class="flex justify-end gap-2">
          ...
          <UButton
            ...
            @click="save"
          >
            {{ editing ? 'Update' : 'Save' }} Approval Check
          </UButton>
        </div>
      </template>
    </UModal>
...
import { useApprovalChecksStore, type ApprovalCheck } from '~/stores/approvalChecks'
...
const editing = ref<ApprovalCheck | null>(null)
const saving = ref(false)
const globalFilter = ref('')

const form = ref({ approval_check: '', description: '', active: true })
...
const columns: TableColumn<ApprovalCheck>[] = [
  {
    id: 'approval-check',
...
function resetForm() {
  form.value = { approval_check: '', description: '', active: true }
}

function edit(item: ApprovalCheck) {
  editing.value = item
  form.value = {
    approval_check: item.approval_check,
...
async function save() {
  ...
  try {
    const payload = {
      approval_check: form.value.approval_check,
      description: form.value.description || null,
      active: form.value.active,
    }
    if (editing.value) {
      await store.updateApprovalCheck(editing.value.uuid, payload)
      toast.add({ title: 'Success', description: 'Approval Check updated successfully', color: 'success' })
    }
    else {
      await store.createApprovalCheck(payload)
...
async function remove(item: ApprovalCheck) {
  try {
    await store.deleteApprovalCheck(item.uuid)
    toast.add({ title: 'Success', description: 'Approval Check deleted successfully', color: 'success' })
  }
  catch (e: unknown) {
    toast.add({ title: 'Error', description: getErrorMessage(e), color: 'error' })
  }
