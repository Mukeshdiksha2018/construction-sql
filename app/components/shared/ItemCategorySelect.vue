  ...
  <div class="flex flex-col gap-1">
    ...
    >
      <template #item-label="{ item }">
        <div
          :class="
            item.value === '__ALL__' || item.isAllOption
...
const optionsMap = computed(() => {
  const map = new Map<string, { value: string; label: string; isAllOption?: boolean }>()
  for (const o of categoryOptions.value) {
    map.set(o.value, o)
  }
  return map
})
...
function handleSelection(item: { value: string; label: string; isAllOption?: boolean } | string | undefined) {
  if (!item) {
    if (props.includeAllOption) {
      selectedObject.value = categoryOptions.value.find((o) => o.value === '__ALL__') ?? undefined
    } else {
