      ...
      <div class="hidden md:block">
        <table class="min-w-full table-fixed divide-y divide-default/60">
          <thead class="bg-muted/20 text-[11px] font-semibold uppercase tracking-wide text-muted">
            <tr>
              <th class="w-1/5 px-4 py-2 text-left"><USkeleton class="h-3 w-20" /></th>
              <th v-if="showLocationColumn" class="w-1/6 px-4 py-2 text-left"><USkeleton class="h-3 w-20" /></th>
              <th class="w-1/6 px-4 py-2 text-right"><USkeleton class="h-3 w-20" /></th>
              <th class="w-1/6 px-4 py-2 text-right"><USkeleton class="h-3 w-20" /></th>
              <th class="w-1/6 px-4 py-2 text-right"><USkeleton class="h-3 w-20" /></th>
              <th class="w-[64px] px-4 py-2 text-right"><USkeleton class="h-3 w-16" /></th>
            </tr>
          </thead>
          <tbody class="divide-y divide-default/60">
            <tr v-for="i in 3" :key="i" class="align-middle">
              ...
              <td v-if="showLocationColumn" class="px-2 py-2"><USkeleton class="h-4 w-24" /></td>
    ...
    <div v-else-if="hasItems">
      <div class="hidden md:block">
        <table class="min-w-full table-fixed divide-y divide-default/60">
          <thead class="bg-muted/20 text-[11px] font-semibold uppercase tracking-wide text-muted">
            <tr>
              <th class="px-4 py-2 text-left" :class="showLocationColumn ? 'w-1/5' : 'w-1/4'">Cost Code</th>
              <th v-if="showLocationColumn" class="w-1/6 px-4 py-2 text-left">Location</th>
              <th v-if="showLocationColumn" class="w-1/6 px-4 py-2 text-left">Description</th>
              <th v-if="showBudgetedAmount" class="px-4 py-2 text-right" :class="showLocationColumn ? 'w-[120px]' : 'w-1/5'">Budgeted Amt</th>
              <th class="px-4 py-2 text-right" :class="showLocationColumn ? 'w-[120px]' : 'w-1/5'">PO Amount</th>
              <th class="px-4 py-2 text-right" :class="showLocationColumn ? 'w-[140px]' : 'w-1/5'">CO Amount</th>
          ...
          <tbody class="divide-y divide-default/60 text-sm text-default">
            ...
            >
              ...
              <td v-if="showLocationColumn" class="px-2 py-2 align-middle text-xs text-default">
                <LocationSelect
              ...
              <td v-if="showLocationColumn" class="px-2 py-2 align-top text-xs text-default max-w-xs">
                <div class="whitespace-pre-wrap break-words text-[12px] leading-snug text-default/90">
                  {{ row.description || '' }}
                </div>
              </td>
              <td v-if="showBudgetedAmount" class="px-2 py-2 text-right">
                <span class="font-mono text-xs text-muted">{{ formatCurrency(row.material_budgeted_amount || 0) }}</span>
              </td>
      ...
      <div class="md:hidden divide-y divide-default/60">
        ...
        >
          ...
          <div v-if="showLocationColumn" class="mt-2 space-y-1 text-xs text-default">
            <div>
              <span class="block text-[11px] uppercase tracking-wide text-muted/80">Location</span>
              <span class="block">{{ row.location_label || row.location_uuid || '—' }}</span>
            </div>
            <div>
          ...
          <div class="grid gap-3 text-xs text-default mt-3" :class="showBudgetedAmount ? 'grid-cols-3' : 'grid-cols-2'">
            <div v-if="showBudgetedAmount">
              <span class="block text-[11px] uppercase tracking-wide text-muted/80">Budgeted Amt</span>
              <div class="font-mono text-muted">{{ formatCurrency(row.material_budgeted_amount || 0) }}</div>
            </div>
            <div>
...
interface COLocationWiseMaterialItemDisplay {
  id?: string | number
...
const props = withDefaults(defineProps<{
  title?: string
  description?: string
  items: COLocationWiseMaterialItemDisplay[]
  loading?: boolean
  error?: string | null
  ...
  loading: false,
  error: null,
  loadingMessage: 'Loading location-wise material items…',
  emptyMessage: 'No location-wise material items found.',
  readonly: false,
